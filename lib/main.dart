...
class DatabaseHelper {
  ...
  Future _onCreate(Database db, int version) async { //создание БД
    await db.execute('''
  ...
  Future<List<Grocery>> getGroceries() async { //метод получения списка
  ...
  Future<int> add(Grocery grocery) async { //добавление продуктов
    Database db = await instance.database;
    return await db.insert('groceries', grocery.toMap());
  }
  Future<int> remove(int id) async { //метод удаления
    Database db = await instance.database;
    return await db.delete('groceries', where: 'id = ?', whereArgs: [id]);
  }

  Future<int> update(Grocery grocery) async { //обновление
    Database db = await instance.database;
